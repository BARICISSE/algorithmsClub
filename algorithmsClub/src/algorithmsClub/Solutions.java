...
public class Solutions {
	...
	public int findKthLargest(int[] nums, int k) {
       ...
       largestKth = sortedArr[nums.length - k];
	...
	//quick sort to help sort the elements
	...
	public List<Interval> merge2(List<Interval> intervals) {
        ...
        for(Interval item: intervals){
            if(item != null && merged.size() > 0 && merged.get(merged.size()-1).end < item.start){
                merged.add(item); 
            }else if(merged.size() > 0){
                 merged.get(merged.size()-1).end = Math.max(item.end, merged.get(merged.size()-1).end);
            }else{
                merged.add(item);
            }
